...
# Generating correlations and bivariate plot
plotlist <- lapply(colnames(combined)[1:19], function(cell){

    # Correlation
    correl <- cor.test(combined$counts, combined[,cell], method = "spearman")
    
    # Linear model to obtain intercept and regression coefficient
    lmodcoeff <- lm(combined[,cell]~combined$counts)$coefficients
    
    # Bivariate plot
    plotclust(data = combined, xvar = "counts", yvar = cell, type = "biplot", col = "darkgray") +
      geom_abline(intercept = lmodcoeff[1], slope = lmodcoeff[2], colour = "red", linewidth = 1) + 
...
The correlations with ESR1 are significant for all cell types except for the endothelial cells. The correlation is always negative except for endothelial cells. However, all correlations are very low, ranging from 0.13 to 0.40. We can conclude that there is weak correlation between ESR1 expression and the abundance scores for the different cell types.
...
# To generate the results for the other classifications, one just has to replace "ER_level" with "wardD2_3", "kmeans3", "wardD2_2", "complete2", or "kmeans_GMM2"
```
...
The two classifications that appear to best distinguish between their categories overall are systematically *kmeans3* (only one exception over the 10 comparisons) and *kmeans_GMM2* These are essentially the ones with the highest effect size (see the interpretation column). These always show a significant adjusted p-value and ordered gradient of medians. It can be observed that on several occasions the adjusted p-value is not even significant in the case of *wardD2_2*. In more half of the cases the IHC classification shows an unordered gradient.
...
### ADDITIONAL COMPARISONS BASED ON THE RESULTS OF VOORWERK ET AL. (2023)

As mentionned previously, here we will generate more comparable boxplots to those shown in Voorwerk et al. (2023: Figs. 1a-c and 2b-i), though not identical due to methodological differences. The objective is to check whether the ivestigated classifications reproduce (at least partially) the trends described there. If this is the case it can be a good argument that the classification is biologically valuable.

For Fig. 1a ("Stromal TILs"), we will combine the scores of the NK cells, CD4 T cells, CD8 T cells, and regulatory T cells and use the mean score.
For Fig. 1b ("T_cells_CD8"), we will simply use the CD8 T cell scores (same plot as the one computed earlier).
For Fig. 1c ("PDL1 expression"), we will use the expression of the CD274 gene.
For the boxplots in Fig. 2b-i, we will use the gene expression or mean expression of the set of genes, corresponding to each molecular signature.

To make visual comparisons easier, jitter points with similar colors are used.

We can apply our custom function *voorwerkplots()* specifically devised for that purpose:

```{r, fig.height=5, fig.width=10, out.width="100%"}
# Comparable boxplots to those in Voorwerk et al. (2023)
voorwerkplots(cluster=colnames(combined)[c(21:26)], data1 = combined, data2 = datamatrix)
```


- Traditional IHC categories:
There is moderate agreement with the results of Voorwerk et al. (2023). The concordance is only moderate because, generally, the "<1%" category has relatively higher values and the "10-50%" category relatively lower values than in the plots of Voorwerk et al. (2023).
However, the trend in our data fits very well with those observed in Voorwerk et al. (2023) for some signatures: regulatory T cells, IFN-γ, inflammatory chemokine (although the values in the "<1%" category are too high), and mast cells.

- Alternative ternary classifications:
It is not straightforward to accurately evaluate the concordance with the trends observed in Voorwerk et al. (2023). However, considering that the "Moderate" category is an intermediate category and thus comparable to a mix of the traditional "1-9%" and "10-50%" categories together—simply for visual trend comparisons since it is not really the case quantitatively speaking—and that the extremes are roughly comparable as well, the trends correspond quite well. It decreases for the abundance of stromal TILs and PDL1 expression (*kmeans3* fits somewhat better), and is maintained before decreasing in the case of the abundance of stromal CD8 cells (*wardD2_3* fits somewhat better). Regarding the molecular signatures, the trend is also maintained before decreasing in the case of the CD8+ T cells, regulatory T cells, and PD1 (*wardD2_3* fits somewhat better for the two first and *kmeans3* for the last). For the APM, IFN-gamma, and tumor inflammation, *kmeans3* fits somewhat better as the decrease is more marked between the first and second categories.*wardD2_3* fits somewhat better for the inflammatory chemokine.
The among-group differences seem generally (somewhat) more marked for *kmeans3*.

- Alternative binary classifications:
The results are extremely similar from one classification to another, though the among-group differences almost systematically appear being somewhat more pronounced for *kmeans_GMM2*. In any case, there is agreement, since the results of Voorwerk et al. (2023) always show a difference between the two most extreme categories, like here.

Though definitive conclusions are not easily drawn, it is interesting to note that the IHC classification is not particularly more congruent trend-wise than the alternative classifications, and yet its number of categories and their definition is much closer, almost identical, to those used in Voorwerk et al. (2023).

In summary, the trends observed in Voorwerk et al., are indeed partially reflected in our data, with some evident variation. Once again, *kmeans3* and *kmeans_GMM2* appear to be good candidate classifications with regards to the underlying biology of breast tumors.
