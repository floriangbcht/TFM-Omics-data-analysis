...
for(k in c(2,3)){
  set.seed(42)
  groups[[paste0("kmeans", k)]] <- factor(kmeans(countsERmod$counts, centers = k, iter.max = 10000, nstart = 100)$cluster)
}
...
groups <- groups %>% mutate(
  ...
  kmeans3=c("Moderate","High","Low")[kmeans3],
  kmeans3=factor(kmeans3, levels=c("Low","Moderate","High"))# Reordering the levels
  ,
  # Kmeans with two groups
  kmeans2=as.factor(c("LowToMod", "ModToHigh")[kmeans2])# No need to reorder the levels
)
...
# Cluster sizes
for(k in c(2,3)){
  cat("Classification:", paste0("kmeans", k))
  print(table(groups[[paste0("kmeans", k)]]))
  cat("\n")
}
```
...
countsERmod <- countsERmod %>% mutate(
  ...
  kmeans2=groups$kmeans2,
  # K-means with three groups
  kmeans3=groups$kmeans3)
```
...
Nice mosaic plots can also be generated:

```{r, fig.height=5, fig.width=10, out.width="100%"}
# Compound mosaic plots for IHC vs alternative ternary classifications
par(mar = c(1, 0, 2, 1)) # Reducing plot margin sizes
par(mfrow=c(1,2)) # Modifying plotting window disposition
for(classif in colnames(countsERmod)[3:4]){
  mosaicplot(table(countsERmod[,classif], countsERmod$ER_level), color=colplotclust("ER_level", countsERmod)$col, las=1, 
                   main=paste("IHC vs.", classif, "\n"), cex.axis = 1)
}

# Compound mosaic plots for IHC vs alternative binary classifications
par(mfrow=c(1,3))  # Modifying plotting window disposition
for(classif in colnames(countsERmod)[5:7]){
  mosaicplot(table(countsERmod[,classif], countsERmod$ER_level), color=colplotclust("ER_level", countsERmod)$col, las=1,
                   main=paste("IHC vs.", classif, "\n"), cex.axis = 1)
}

# Reinitializing plot margin sizes and plotting window disposition
par(mfrow=c(1,1))
