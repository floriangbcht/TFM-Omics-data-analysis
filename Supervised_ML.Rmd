...
First of all, we observe that ESR1 expression ("counts") is always the most important feature and shows a relatively much higher mean SHAP value than the other features. This is especially true for the alternative classifications and when the algorithm employed is RF or XGBoost (most marked for XGBoost). The only exception is for the "10-50%" category ("ModPos" here) of the traditional IHC classification for which ESR1 expression is the third most important feature (but its SHAP value is not very different than those of the two most important features).
...
# Heatmap
pheatmap::pheatmap(overallshap, border_color=NA, fontsize_col=13, angle_col=45, color = heatcol)
```
